 ...
 */
export function validateBlockTree(root: BlockLike | undefined): BlockIssues {
  const issues: BlockIssues = new Map();
  if (root) walk(root, [], issues);
  return issues;
}

function walk(block: BlockLike, path: BlockPath, out: BlockIssues): void {
  ...
  for (const child of children) walk(child.block, [...path, ...child.path], out);
}
